...
function TimePicker({ bookings, roomNumber }: Props) {

    const [value, setChange] = React.useState(new Date());
    const [endvalue, endSetChange] = React.useState(new Date());
    const [avalilbaleSlots, setAvaliableSlots] = React.useState<Date[]>([]);
    const [excludeDate, setExcludeDate] = React.useState<Date>(new Date());
    function getExcludedTimeSlots(bookings: bookings[], roomFilter: string): Date[] {
        ...
        for (const booking of bookings) {
            ...
            if (booking.roomNumber === roomFilter || booking.roomType === roomFilter) {
        ...
        // also exclude all the time before current time
    ...
    const handleStartChange = (date: Date | null, event: React.SyntheticEvent<any, Event> | undefined) => {
        ...
        setAvaliableSlots(getExcludedEndTimeSlots(bookings, roomNumber, date!));
    }
    ...
    return (
        <div className='flex space-x-2'>
            <DatePicker
                ...
                minDate={new Date()}
                // exclude all the time before current time and get the excluded time slots
                excludeTimes={getExcludedTimeSlots(bookings, roomNumber)}
                excludeDateIntervals={[{
                    start: new Date(value.getFullYear(), value.getMonth() - 5, value.getDate()),
                    end: new Date()
                }]}
                timeFormat="p"
                timeIntervals={60}
...

            {
                ...
                <DatePicker
                    ...
                    selected={endvalue}
                    className="block text-white p-2 mt-1 bg-gray-600 rounded-lg shadow-sm border-gray-300 focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                    onSelect={(date) => endSetChange(date!)}
                    onChange={(date) => endSetChange(date!)}
                    minDate={new Date()}
                    //exclude all the date range after exclude date
                    excludeDateIntervals={[{
                        start: excludeDate,
                        end: new Date(9999, 12, 31)
                    }]}
                    // exclude all the time after the avaliableSlots
                    excludeTimes={avalilbaleSlots}
                    timeFormat="p"
                    timeIntervals={60}
                    dateFormat="Pp"
                />}
